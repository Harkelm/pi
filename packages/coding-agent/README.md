...
Compaction is lossy. The full history remains in the JSONL file; use `/tree` to revisit. Pi frames compaction and branch summaries as generated context, not direct user authority. Customize compaction behavior via [extensions](#extensions). See [docs/compaction.md](docs/compaction.md) for internals.
